...
| `SDI_SW_FLATTEN=0` | Stop measuring the blank by flattening a formed part in memory; the blank is inferred instead. Default: **on**. This is the one measurement only a seat can make — reading the cut-list property cannot tell a flat pattern from a weldment's folded envelope, because SolidWorks names both `Bounding Box Length`, and on 12120-01-01M that under-bought the blank by 6mm in each axis. It fires only on a formed sheet-metal part with no usable cut-list blank, and never on a document somebody already has open |
...
| `SDI_SCAN_SOURCE_DIR` | The folder the scanner writes the day's delivery notes into, for `tools/logistics/split_delivery_notes.py`. **No default, on purpose.** A guessed one was carried briefly, reasoned from the estimating share and the drive letter in Explorer; it was wrong, and the job *created* that folder tree rather than refusing it — after which the path existed, `Test-Path` answered True, and the diagnosis went looking at `Path.glob`. Give it as a UNC path, never `K:\…`: a mapped drive belongs to a logged-on session, and the scheduled task, the backend service and an elevated shell each lack one |
| `SDI_SCAN_SPLIT_DIR` | Where the splitter writes one PDF per delivery note (the `SplitScan` folder). Same rules; the splitter creates this folder but never the tree above it |
| `SDI_LLM_ONLY=1` | What `--llm-only` sets for the process: the BOM comes from the vision model alone — deterministic BOM reader, DXF flat patterns and SolidWorks extract all off. A measurement of the model, labelled as such on every deliverable; on a pack that yields no parts (a customer render), the concept read sights them and the ordinary waterfall prices them. Set by `main.py`, not by hand |
| `SDI_VISION_REFRESH=1` | What `--fresh-read` sets: both LLM caches bypassed, every page goes to the model again. For "what does the model say TODAY", never for an estimate — the caches exist because 2085 came back with and without welding on consecutive runs |
| `SDI_OFFLINE=1` | No network: pricing sources, the vision reads and the concept read all refuse by name instead of reaching out. The test suite's standard mode |
| `SDI_TESSERACT_PATH` | Full path to `tesseract.exe` for the delivery-note splitter, when it is not on PATH and not in the usual install folders. Leave unset to auto-detect — PATH first, then `C:\Program Files\Tesseract-OCR` and the rest, the same order `cad_inputs.find_converter` uses for the ODA converter. UB-Mannheim's installer does **not** add it to PATH, so `winget install` completes, reports success, and `tesseract` is still "not recognized"; a PATH edit also needs a new shell before it takes |
| `SDI_WELD_DEBUG=1` | Per-weld diagnostics from the document builder |
| `SDI_DB_PASSWORD` | **Required.** Password for the SDILive login. SDILive is Access Supply Chain's primary database — Sage X3 is not live. There is deliberately **no default**: it was a string literal in seventeen source files, so changing it meant editing source, which meant committing it. An unset password fails by name rather than falling back. Goes in `src/.env`, which is gitignored, and never in source |
| `SDI_DB_SERVER` / `SDI_DB_NAME` / `SDI_DB_USER` | SDILive host, database and login (defaults `10.0.0.200` / `SDILive` / `AIBot`). Not secrets — an internal address and a service-account name — so they keep defaults and an existing machine goes on working |
| `SDI_DB_DRIVER` | ODBC driver name (default `ODBC Driver 18 for SQL Server`). **SDI-APP01 carries only Driver 17**, and asking for one that is not installed reports `IM002`, a client-side error that reads exactly like the server being unreachable. Set it per machine |
...
| `SDI_PRINT_OFFICE` | Set to `1` to include Word, Excel and PowerPoint documents when printing a drawing pack. **Off by default, deliberately.** Converting them drives the Office application itself through COM, which can stall on a dialog this process cannot see — a Protected View banner, a file lock, a repair prompt. It hung the first two real prints. Everything else Print handles (PDF, DXF, DWG, text, images) is a library call that cannot block like that, so the default keeps Print reliable and makes the fragile path a deliberate choice. Needs an interactive desktop, so it works on the estimating laptop and not on a session-0 service |
...
| `SDI_DM_OUTPUT_ROOT` | *(portal backend)* Folder the Document Manager extract tool writes its packs to. Must ALSO appear in `SDI_FILE_ROOTS` — there is no looser path rule for this feature |
| `SDI_DM_API_BASE` | *(portal backend)* Base URL of the Document Manager API, e.g. `http://<dm-host>:8000`. `DOCMGR_BASE_URL` is accepted too, so a machine set up from the DM team's own guide already works |
| `SDI_DM_API_KEY` | *(portal backend)* The shared `X-API-Key` access secret for that API. `DOCMGR_ACCESS_SECRET` is accepted too. **Backend only — never in page JavaScript.** It is one secret covering every consumer, so anything that can read it can drive the CAD host. Put it in `.env`, which is gitignored; it must not be committed |
| `SDI_DRIVE_MAP` | *(portal backend)* Mapped drive letters rewritten to the share they stand for, as `K=\\server\share` pairs separated by `;` (default `K=\\sdi-dc01\shareddata$\Shared`). Applied only to paths arriving from **another machine** — today the Document Manager's `outputDir`. A letter is a per-logon-session mapping, means nothing to a service, and is not the form `SDI_FILE_ROOTS` is written in, so an otherwise readable folder fails both the containment check and the open. A letter with no mapping is left alone rather than guessed at |
| `SDI_DM_HEALTH_TIMEOUT` / `SDI_DM_START_TIMEOUT` / `SDI_DM_POLL_TIMEOUT` | *(portal backend)* Seconds for the three DM calls (defaults **10** / **60** / **30**). An extract is minutes of COM work but is asynchronous — the portal starts it and polls, so no request is held open for it |
