...
# Assembly-note bought-in scanner (note_scan.scan_notes_for_bought_in). Finds loose
# bought-in components mentioned in assembly-page PROSE (junction box, mains cable,
# earth strap, adhesive cable clip, foam tape, etc.) that never appear as BOM rows, so
# the BOM-row extractor cannot see them. Every item it returns is flagged
# review_flag=True ("AI-identified — verify item, qty & price"); it never silently adds
# a confident line. Reconciled against items already found, so it cannot double-count
# the loom/fixings. Uses the LLM as a finder over text the engine already extracted.
NOTE_SCAN_POLICY = {
    "enable": True,
}
