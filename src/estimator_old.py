...
def extract_bought_in_from_pages(
    ...
    *,
    ...
    # ── Supplementary: LLM note-scan (additive only, gated, reconciled) ───────
    # Deterministic extraction above is the source of truth. The note-scan runs SECOND,
    # on the same joined note-text, and can only ADD bought-in items that are described in
    # prose notes but never appear as a BOM row (cable clips, earth strap, mains cable,
    # downlights, ...). It reconciles against everything already found (existing_pns,
    # seen_codes, and the descriptions captured so far) so it never duplicates or conflicts.
    # Gated behind config.NOTE_SCAN_POLICY['enable']; if off or absent, this is a no-op and
    # the engine behaves exactly as before. Never raises into this function.
    try:
