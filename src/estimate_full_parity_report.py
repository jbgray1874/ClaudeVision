...
def _parity_money_specs() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "context_must_include_any": ["subtotal", "sub total", "total"],
            "exclude_if_any": ["material", "sell", "hours", "dept", "department", "by dept"],
            "pick": "last",
            "fallback_read_columns": ["M", "L"],
            "value_scan_span": 14,
        },
...
def _build_labour_route_comparisons(
    ...
    *,
    ...
    for canonical in all_canonical:
        ...
        # If the manual estimate's per-operation £ wasn't extracted from the workbook (reads 0/None),
        # we can't magnitude-compare this op — show the route mapping as neutral "info", NOT a
        # fabricated "Fail vs £0". The real magnitude verdict lives in the money cells (Section 6).
        cost_wb_valid = isinstance(cost_wb, (int, float)) and cost_wb > 0
        if not cost_wb_valid:
            worst = "info"
        elif hrs_json is None and cost_json is None:
            worst = "review"
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "bom_set_reconciliation": build_bom_set_reconciliation(summary, ws, *_sheet_scan_bounds(ws)),
...
def _norm_line_code(raw: Any) -> str:
...
def _ai_bom_lines(summary: Dict[str, Any]) -> List[Dict[str, Any]]:
    """All AI estimate lines for set reconciliation — fabricated parts + bought-in catalogue."""
    est = summary.get("estimate_summary") or {}
    lines: List[Dict[str, Any]] = []
    # Fabricated parts
    for pe in _parts_from_estimate(est):
        code = _norm_line_code(pe.get("part_number"))
        if not code:
            continue
        total = _safe_float(pe.get("total_cost_gbp")) or _safe_float(
            (pe.get("cost_breakdown") or {}).get("total_cost_gbp")
        )
        lines.append({
            "code": code,
            "description": str(pe.get("description") or "").strip(),
            "qty": _safe_float(pe.get("quantity")) or 1,
            "cost_gbp": total,
            "kind": "fabricated",
        })
    # Bought-in catalogue lines from bay_estimate
    bay = summary.get("bay_estimate") or {}
    for ln in (bay.get("lines") or []):
        if str(ln.get("kind") or "") != "catalogue":
            continue
        code = _norm_line_code(ln.get("code"))
        if not code:
            continue
        lines.append({
            "code": code,
            "description": str(ln.get("description") or "").strip(),
            "qty": _safe_float(ln.get("qty")) or 1,
            "cost_gbp": _safe_float(ln.get("line_cost_gbp")),
            "kind": "bought_in",
        })
    return lines


def _manual_bom_lines(ws: Any, max_row: int, max_col: int) -> List[Dict[str, Any]]:
    """Extract BOM lines from the manual workbook by scanning for part-code-like cells
    with a description and a value to the right. Format-tolerant — works across the
    different manual estimate layouts the five estimators use."""
    lines: List[Dict[str, Any]] = []
    seen: set = set()
    ...
    for row in range(1, max_row + 1):
        # Look for a code-like cell in cols A-F
        code_val = None
        desc_val = ""
        ...
        if not code or code in seen or len(code) < 3 or not any(ch.isalpha() for ch in code):
            continue
        # Find a money value to the right
        cost = None
        for col in range(2, max_col + 1):
            ...
            if f is not None and f > 0:
                cost = f  # last positive number on the row = line total
        seen.add(code)
        lines.append({
            "code": code,
...
# Lines the engine NEVER fabricates from a drawing — added by the estimator at quote time.
# These appearing as "manual only" is BY DESIGN, not a miss.
_OUT_OF_SCOPE_RE = re.compile(
    r"\b(DELIVERY|CARRIAGE|FREIGHT|HAULAGE|TRANSPORT|SHIPP|PALLET|"
    r"PACKAGING|PACKING|EURO\s*PALLET|MISC\s*PACKAGING|"
    r"OVERHANG|ARTIC|CRATE)\b",
    re.IGNORECASE,
)
...
def build_bom_set_reconciliation(
    summary: Dict[str, Any],
    ws: Any,
    max_row: int,
    max_col: int,
) -> Dict[str, Any]:
    """Three-way BOM reconciliation: matched (both), manual-only (AI missed),
    AI-only (AI extra). The manual estimate may be by any of the estimating team."""
    ai_lines = _ai_bom_lines(summary)
    manual_lines = _manual_bom_lines(ws, max_row, max_col)

    ai_by_code = {ln["code"]: ln for ln in ai_lines}
    ...
    matched, manual_only, ai_only = [], [], []

    for code in sorted(manual_codes & ai_codes):
        m, a = manual_by_code[code], ai_by_code[code]
        mc, ac = m.get("cost_gbp"), a.get("cost_gbp")
        pct = _pct_diff(mc, ac) if (mc and ac) else None
        matched.append({
            "code": code,
            "description": a.get("description") or m.get("description"),
            "manual_cost_gbp": mc,
            "ai_cost_gbp": ac,
            "variance_pct": pct,
            "kind": a.get("kind"),
    ...
    for code in sorted(manual_codes - ai_codes):
        ...
        manual_only.append({
            ...
            "manual_cost_gbp": m.get("cost_gbp"),
            "category": _cls["category"],
            "issue": _cls["issue"],
        })

    _stem_matched_ai = set(_stem_pairs.values())
    for code in sorted(ai_codes - manual_codes):
        ...
        ai_only.append({
            ...
            "ai_cost_gbp": a.get("cost_gbp"),
            "kind": a.get("kind"),
            "issue": "AI estimate has a line the manual estimate does not — verify",
        })

    return {
        "schema": "bom_set_reconciliation.v1",
        "manual_line_count": len(manual_lines),
        "ai_line_count": len(ai_lines),
        "matched_count": len(matched),
        "manual_only_count": len(manual_only),
        "genuine_miss_count": sum(1 for r in manual_only if r.get("category") == "genuine_miss"),
        "out_of_scope_count": sum(1 for r in manual_only if r.get("category") == "out_of_scope"),
        "ai_only_count": len(ai_only),
        "match_rate_pct": round(100.0 * len(matched) / len(manual_lines), 1) if manual_lines else None,
        "matched": matched,
