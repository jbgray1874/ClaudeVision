#!/usr/bin/env python3
r"""
_build_bodycare_quote_v3.py

Client-facing WeAreSDI-branded quotation for Bodycare (job 12439-01-13).

v3 change: the unit image (rendered from the GA PDF) now sits as a "Drawing reference" block
JUST ABOVE THE FOOTER, not in the header. Header is logos only. Everything else shifts up.

Also in this build:
  - contact email: matt.evans@wearesdi.com
  - both real logos inlined from C:\Logos
  - unit image rendered from the GA PDF, auto-cropped, embedded base64 (general: change PDF_PATH
    per job). Set PDF_PATH = "" to omit the image entirely (clean logos-only quote).
  - sell price: set QUOTE["sell_price"] to quote a real price; None shows the workbook COST + a
    warning (do not send showing cost).

Requires PyMuPDF (fitz) or pdf2image+poppler, plus Pillow, for the image. If neither renderer is
present it prints the pip line and still builds the document WITHOUT the image.
...
Usage (on the machine with the files):
    C:\ClaudeVision\.venv\Scripts\python.exe _build_bodycare_quote_v3.py
"""
...
QUOTE = {
    ...
    "sell_price": None,          # <-- SET to the client sell price, e.g. 4.20; None shows cost
    "currency":   "£",
...
def _render_pdf_image(pdf_path: str, max_w_px: int = 900) -> str | None:
    if not pdf_path or not os.path.exists(pdf_path):
        if pdf_path:
            ...
            print("  [image] PDF_PATH empty — building clean logos-only quote (no image).")
        return None
    ...
    try:
        ...
        try:
            ...
            print("  [image] Install a renderer:")
...
def build():
    ...
    # Drawing-reference block — now sits ABOVE THE FOOTER (v3)
    unit_img_html = ""
    if unit_uri:
        unit_img_html = f'''
      <div class="unit-view">
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  /* drawing reference — above footer */
...
"""
    ...
    if q["is_cost"]:
        print(f"  *** SHOWING COST £{q['unit_cost']:.2f} (margin 0%). Set QUOTE['sell_price'] before sending. ***")
    print("  Open in a browser; File > Print > Save as PDF to send.")
