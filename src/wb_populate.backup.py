...
CELL_MAP = {
    ...
    "steel": {
        "first_row": 38, "last_row": 48,          # 11 slots
    ...
    "other_sheet": {
        "first_row": 51, "last_row": 58,          # 8 slots
    ...
    "labour": {
        "first_row": 63, "last_row": 134,         # widened block (was 102); 72 slots
...
def populate_workbook(summary: Dict[str, Any], job_folder_name: str) -> Optional[str]:
    ...
    for pe in pes:
        ...
        # 1. drop commercial placeholders
        if pn in DROP_CODES:
            continue
    ...
    for pe in bom_parts:
        ...
        # description: for tube sections, prefer the catalogue description (has size+length)
    ...
    for pe in labour_parts:
        ...
        for op in ops:
            ...
            ws.cell(row=row, column=lb["col_desc"],      value=desc)
            ...
            if bh and bh > 0:
                ...
                throughput = total_pieces / bh
                ws.cell(row=row, column=lb["col_throughput"], value=round(throughput, 4))
            else:
                _flag(f"labour op '{op}' ({pe.get('part_number')}) has no batch_hours — "
