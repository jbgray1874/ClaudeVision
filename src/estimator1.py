...
def _resolve_part_system_cost(part: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # None-PN dedup: suppress phantom None-PN entries — canonical-PN carries the price
    _pn_raw_check = str(part.get("part_number") or "").strip()
    if _pn_raw_check in ("", "None") and (
        part.get("_bought_in_from_text_scan") or part.get("page_roles") == ["bought_in"]
    ):
        return {"result": {}, "applied_unit_cost": None, "matched_part_code": None}

    # Catalogue-first for BOM text-scan bought-in parts
    _is_bom_scan = (
        part.get("_bought_in_from_text_scan")
        or part.get("page_roles") == ["bought_in"]
    )
    if _is_bom_scan and part_code:
        try:
            from pricing_service import PricingService as _PS
            _cat = _PS()._get_catalogue_part(part)
            if _cat and _cat.get("unit_price_gbp") is not None:
                _price = float(_cat["unit_price_gbp"])
                if _price > 0:
                    return {
                        "result": {"selected": {"price": _price, "unit": "each",
                                                "source": "AIEstimating.vCurrentBoughtIn",
                                                "confidence": float(_cat.get("confidence") or 0.95)}},
                        "applied_unit_cost": _price,
...
def estimate_material(part: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # GA title-block fragment guard: PN like "B - 17" or "A - FIRST" is a revision
    # letter + drawing number fragment extracted from the cover sheet. Not a part.
    _pn_raw = str(part.get("part_number") or "").strip()
    if re.match(r"^[A-Z]\s*[-–]\s*(\d+|FIRST|SECOND|THIRD)$", _pn_raw, re.IGNORECASE):
        return {
            "material": "GA_FRAGMENT", "unit_material_cost_gbp": 0.0,
            "cost_per_part_gbp": 0.0, "extended_material_cost_gbp": 0.0,
            "extended_sheet_material_cost_gbp": 0.0, "unit_material_mass_kg": None,
            "blank_length_mm": None, "blank_width_mm": None, "blank_area_m2": None,
            "thickness_mm": None, "powder_consumable": None,
            "stock_estimate": {"candidate_sheet_size_mm": None, "parts_per_sheet": None, "utilisation_pct": None},
            "cost_method": "ga_fragment_suppressed",
            "part_confidence_overall": 0.0, "part_geometry_reliability": 0.0,
            "reliability_flags": ["ga_title_block_fragment_suppressed"],
            "note": "GA/title-block fragment — suppressed to £0.",
            "price_source": _build_price_source_metadata(
                {}, fallback_source="ga_fragment_suppressed", applied=False, applied_basis=None),
        }
    ...
    if _mat_acr in {"ACRYLIC", "HIGH IMPACT ACRYLIC", "PERSPEX", "PMMA", "POLYCARBONATE"} and blank_length and blank_width:
        _acr_prices = getattr(config, "ACRYLIC_SHEET_PRICE_GBP", {}) or {}
        try:
        ...
        return {
            ...
            "price_source": _build_price_source_metadata(
                {}, fallback_source="acrylic_sheet_provisional",
    ...
    # DXF stated-weight guard: for steel/metal parts with a DXF flat pattern,
    # the drawing's spec-block WEIGHT is the finished assembly weight (includes
    # all children, hardware, finish) — NOT the blank. Suppress it and let the
    # DXF area formula compute mass instead.
    # IMPORTANT: do NOT suppress for TIMBER/MFC/BOARD — those parts are single
    # blanks, not assemblies, so the DXF blank dimensions give the correct blank
    # and the stated weight may still be useful as a sanity check.
    _mat_upper_sw = str(part.get("normalized_material") or "").upper().replace("_", " ")
    _is_metal_part = _mat_upper_sw in {
        "MILD STEEL", "MILD_STEEL", "STAINLESS STEEL", "STAINLESS_STEEL",
        "ALUMINIUM", "ALUMINUM", "ZINTEC", "GALVANISED STEEL", "GALVANISED_STEEL",
    }
    _dxf_backed = (
        str(part.get("geometry_source") or "").lower() in {"dxf_flat_pattern", "dxf"}
        or bool(part.get("dxf_augmented"))
        or bool(part.get("dxf_weight_g"))
        or bool(part.get("dxf_weight_kg"))
    )
    if _dxf_backed and _is_metal_part:
        # Metal + DXF: spec-block weight is assembly weight — use DXF geometry only
        _dxf_g = _safe_float(part.get("dxf_weight_g"))
        _dxf_kg = _safe_float(part.get("dxf_weight_kg"))
        if _dxf_g and _dxf_g > 0:
            stated_weight_kg = round(_dxf_g / 1000.0, 4)
        elif _dxf_kg and _dxf_kg > 0:
            stated_weight_kg = round(_dxf_kg, 4)
        else:
            stated_weight_kg = None  # let area formula take over
    else:
        # Board/timber/acrylic or non-DXF: use normal stated weight path
...
def estimate_process_times(part: Dict[str, Any], quantity: int = 1) -> Dict[str, Any]:
    ...
    if _section_no_dxf:
        ...
        # Also clean the DISPLAYED route. The cost calc above already excludes laser/saw
        # (they're filtered out of local `ops`), but textual_operations is what the estimator
        # reads as the manufacturing route — leaving laser_cutting there is misleading (an
        # estimator sees "laser" on a bought-in tube and questions it). Strip it so the
        # displayed route matches the costed route. Cost is unchanged; this is display-only.
...
def estimate_part(part: Dict[str, Any], job_quantity: Optional[int] = None) -> Dict[str, Any]:
    ...
    _ep_pn_wm = str(part.get("part_number") or "").upper()
    _ep_desc_wm = str(part.get("description") or "").upper()
    _ep_has_own_flat = "dxf" in str(part.get("geometry_source") or "").lower()
    _ep_wm_tokens = getattr(config, "WELDMENT_PARENT_DESC_TOKENS",
                             ["WELDMENT", "WELD ASSEMBLY", "WELDED ASSEMBLY"])
    _ep_wm_suffixes = getattr(config, "WELDMENT_PARENT_PN_SUFFIXES", [r"-WA\d*$", r"-SA\d*$"])
    _ep_is_weldment = (
        any(_t.upper() in _ep_desc_wm for _t in _ep_wm_tokens)
        or (not _ep_has_own_flat and any(re.search(_sfx, _ep_pn_wm) for _sfx in _ep_wm_suffixes))
        or bool(part.get("is_assembly_parent"))
    )
    if _ep_is_weldment:
        _PARENT_FAB_STRIP = {
    ...
    if bought_in_candidate and system_unit_cost is not None:
        _price_source = str(
            (system_cost.get("result") or {}).get("selected", {}).get("source") or ""
        )
        _is_catalogue_price = "vCurrentBoughtIn" in _price_source or "BoughtInCatalogue" in _price_source
        _fitting_cost = 0.0 if _is_catalogue_price else (
            (float(getattr(config, "BOUGHT_IN_FITTING_MIN_PER_PART", 2.0) or 2.0) / 60.0)
            * float((HOURLY_RATES_GBP or {}).get("handling", 31.18))
        )
...
# ── Drawing-sourced bought-in BOM capture ────────────────────────────────────
# Captures alphabetic-prefixed BOM rows the SDI/GA parsers structurally skip
# (e.g. ELECTRICS, FIXING5, FIXING 236, FIXING 125). Validated against the real
# 1282 BOM text (pages 1/10/11/20/21). FIXING* codes are handled here, not via
# document_builder Fix B, which treats them as reference-like and drops them.
...
_BOUGHT_IN_STOP_EXACT = {
    ...
    "BLACK", "COUNTRY", "DEPENDENT", "EARTH", "STRAP", "PITCH",
}
# Header-fragment prefixes (e.g. "ITEMDWG" from a header printed with no space).
...
# Positive-shape gate for captured CODES. A real bought-in code carries a digit
# (FIXING5, FIXING236, WSF45) or is a known pure-alpha bought-in token. Plain
# words (MAKE, EARTH, BLACK) and title-block dates (NOVEMBER / NOVEMBER2013) are
# drawing prose, not parts. _BOUGHT_IN_ALPHA_ALLOW is the tuning point — add any
# pure-alpha bought-in codes SDI actually uses.
_BOUGHT_IN_ALPHA_ALLOW = {"ELECTRICS", "MINIFIX", "LOOM"}
_BOUGHT_IN_MONTHS = {
...
def _extract_bought_in_table_rows(
    ...
    item-no / code / description / qty shape, then screened by stop-lists.
    Returns _bought_in_part_stub records tagged source='drawing_bom_scan'.
    """
    captured: List[Dict[str, Any]] = []
    for page in pages:
        text = _page_text_for_bought_in_scan(page)
        if not text:
            continue
        upper = text.upper()
        ...
        for m in _BOUGHT_IN_ROW_RE.finditer(text):
            ...
            if len(code_key) < 3:                        # single letters, "GA", noise
                continue
            # Positive-shape gate: drop title-block dates and prose words the
            # regex shape can't tell from a code (MAKE, NOVEMBER, NOVEMBER2013).
            if re.sub(r"\d+", "", code_key) in _BOUGHT_IN_MONTHS:
                continue
            if (not any(ch.isdigit() for ch in code_key)
                    and code_key not in _BOUGHT_IN_ALPHA_ALLOW):
                continue
...
def extract_bought_in_from_pages(
    ...
    *,
    ...
    Primary pass: structured BOM table-row capture (drawing-sourced), recovering
    alphabetic-prefixed codes (ELECTRICS, FIXING 236, FIXING 125, ...) that the
    SDI/GA parsers skip. Secondary passes: the original narrow FIXING/MINIFIX/
    WOODEN DOWEL regexes, kept as belt-and-braces (deduped).
    """
    ...
    # ── LAYER 2: deterministic prose recogniser (runs BEFORE the LLM) ─────────
    # Reads THIS drawing's note text, matches recognised bought-in component types against
    # SDI's mined vocabulary, and prices confident matches from real history. Deterministic:
    # same drawing -> same items -> same prices, every run. Additive; reconciles against the
    # layer-1 grid finds above. Never raises into this function.
    try:
        import bought_in_recogniser as _bir
        import config as _cfg_l2
        if (getattr(_cfg_l2, "BOUGHT_IN_RECOGNISER_POLICY", {}) or {}).get("enable", True):
            _l2_existing = {str(b.get("description") or "") for b in bought_in}
            _l2_existing |= {str(p.get("description") or "") for p in existing_parts}
            _l2_items = _bir.recognise_bought_in_in_prose(
                all_text,
                get_connection=_cfg_l2.get_connection,
                existing_pns=existing_pns,
                existing_descriptions=_l2_existing,
                stub_builder=_bought_in_part_stub,
            )
            for _it in (_l2_items or []):
                bought_in.append(_it)
                # feed layer-2 finds into the dedup sets so the LLM backstop won't re-add them
                _pn = _it.get("part_number")
                if _pn:
                    existing_pns.add(_pn)
                    seen_codes.add(_pn)
    except Exception:
        pass

    # ── LAYER 3: LLM note-scan (BACKSTOP — additive, gated, reconciled) ───────
    # Deterministic extraction (layers 1 & 2) is the source of truth. The LLM runs LAST, on
    # the same joined note-text, and can only ADD prose-described bought-in items that layers
    # 1 & 2 missed (genuinely novel phrasings not yet in the vocabulary). It reconciles against
    # everything already found (existing_pns, seen_codes, descriptions) so it never duplicates.
    # Gated behind config.NOTE_SCAN_POLICY['enable']; off/absent -> no-op. Never raises here.
    try:
        import note_scan as _ns
        _existing_descs = {str(b.get("description") or "") for b in bought_in}
        _existing_descs |= {str(p.get("description") or "") for p in existing_parts}
        _note_items = _ns.scan_notes_for_bought_in(
            all_text,
            existing_pns=existing_pns,
...
def estimate_document(parts: List[Dict[str, Any]], summary: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    mode = _rounding_mode()
    # Bought-in items (page_role 'bought_in', or flagged from the BOM text scan) are priced
    # in the bay_estimate bought-in section (system_cost / web-LLM) and reconciled into the
    # total there. They ALSO get part_estimate records (with a generic per-part fallback ~£1
    # that is NOT their real catalogue price), so summing them here too DOUBLE-COUNTS the
    # bought-in. Exclude them from the document-total sum; the bay_estimate reconciliation is
    # the single source of truth for bought-in cost.
    def _is_bought_in_record(p: Dict[str, Any]) -> bool:
        if p.get("page_roles") == ["bought_in"] or p.get("_bought_in_from_text_scan"):
            return True
        return False

    _doc_parts = [p for p in part_estimates if not _is_bought_in_record(p)]
    if mode == "per_line":
        document_total_raw = sum(float(item.get("extended_total_cost_gbp") or 0.0) for item in _doc_parts)
    else:
        document_total_raw = sum(float(item.get("extended_total_cost_raw_gbp") or item.get("extended_total_cost_gbp") or 0.0) for item in _doc_parts)
    if mode == "per_section":
