...
def add_provenance_sheet(wb, summary: Dict[str, Any],
    ...
    # IN CHARACTERS EVERY VIEWER HAS. This line carried five emoji — coloured circles, a
    # banknote and an hourglass — and they are astral-plane codepoints that Excel's own
    # Calibri has no glyph for. On the estimators' machines the line rendered as boxes, and
    # a reviewer reported "visible replacement characters in headings and CURRENCY LABELS":
    # the currency label was an emoji pound note. The counts are the content, so nothing is
    # lost by saying them in words — and a status a reader cannot see is not a status.
    cell(row, 1,
