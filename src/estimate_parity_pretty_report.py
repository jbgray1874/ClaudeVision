...
def _line_kind_phrase(kind: str) -> str:
    """Human phrase for a line kind."""
    k = str(kind or "").lower()
    if k == "fabricated":
        return "a fabricated part (cut, folded and finished in-house)"
    if k == "bought_in":
        return "a bought-in item (sourced from a supplier, priced from Access Supply Chain)"
    return "an item"


def _matched_sentence(r: Dict[str, Any]) -> str:
    """One readable sentence explaining a matched line and its variance."""
    desc = _esc(r.get("description") or r.get("code") or "this line")
    mc, ac = r.get("manual_cost_gbp"), r.get("ai_cost_gbp")
    pct = r.get("variance_pct")
    if mc is None or ac is None:
        return f"Both the estimator and the engine included {desc}. A side-by-side cost could not be computed because one side has no line value."
    base = (f"Both the estimator and the engine included {desc}. "
            f"The estimator had it at {_fmt_gbp(mc)}, the engine at {_fmt_gbp(ac)}")
    if isinstance(pct, (int, float)):
        if abs(pct) <= 5:
            return base + f" — within {abs(pct):.1f}%, a close agreement."
        if abs(pct) <= 15:
            return base + f" — a {pct:+.1f}% difference, worth a glance but not alarming."
        return base + f" — a {pct:+.1f}% difference, which is wide enough to check why."
    return base + "."


def _miss_sentence(r: Dict[str, Any]) -> str:
    """One readable sentence explaining a line the engine missed."""
    desc = _esc(r.get("description") or r.get("code") or "this line")
    cost = r.get("manual_cost_gbp")
    cost_txt = f" The estimator priced it at {_fmt_gbp(cost)}." if cost else ""
    return (f"The estimator included {desc}, but the engine did not produce it.{cost_txt} "
            "This is something that should be derivable from the drawing — a part, fixing, "
            "electrical item or tube — so it points to a gap in how the engine read the drawing. "
            "Worth investigating: check whether the line appears in the drawing's bill of materials "
            "and whether the part code exists in Access Supply Chain.")


def _scope_sentence(r: Dict[str, Any]) -> str:
    """One readable sentence explaining an out-of-scope logistics line."""
    desc = _esc(r.get("description") or r.get("code") or "this line")
    cost = r.get("manual_cost_gbp")
    cost_txt = f" ({_fmt_gbp(cost)})" if cost else ""
    return (f"The estimator added {desc}{cost_txt}. This is a logistics or packaging cost — "
            "delivery, pallets, carriage or boxes — which cannot be read from an engineering "
            "drawing. The engine deliberately leaves these to the estimator, so its absence here "
            "is correct, not a fault.")


def _ai_only_sentence(r: Dict[str, Any]) -> str:
    """One readable sentence explaining a line only the engine produced."""
    desc = _esc(r.get("description") or r.get("code") or "this line")
    cost = r.get("ai_cost_gbp")
    cost_txt = f" at {_fmt_gbp(cost)}" if cost else ""
    kind = _line_kind_phrase(r.get("kind"))
    return (f"The engine produced {desc}{cost_txt} — {kind} — but it is not on the manual estimate. "
            "Two possibilities: the engine correctly spotted something the estimator left off, or it "
            "has read a line that should not be there. Worth a quick check against the drawing to "
            "decide which.")


def _build_reconciliation_section(recon: Dict[str, Any]) -> str:
    """Three-way BOM reconciliation, written in plain English so a reviewer can read it
    like a hand-over note, not decode a table of codes."""
    if not recon:
        return "<p class='muted'>No line-by-line reconciliation is available for this job — the manual workbook BOM could not be read.</p>"

    matched = recon.get("matched") or []
    manual_only = recon.get("manual_only") or []
    ai_only = recon.get("ai_only") or []
    match_rate = recon.get("match_rate_pct")

    genuine_miss = [r for r in manual_only if r.get("category") != "out_of_scope"]
    out_of_scope = [r for r in manual_only if r.get("category") == "out_of_scope"]

    # ── Plain-English summary paragraph — the "read this first" narrative ──
    n_match, n_miss, n_scope, n_extra = len(matched), len(genuine_miss), len(out_of_scope), len(ai_only)
    rate_txt = f"{match_rate:.0f}%" if isinstance(match_rate, (int, float)) else "—"

    summary_bits = []
    summary_bits.append(
        f"The engine and the estimator agreed on <strong>{n_match}</strong> "
        f"line{'s' if n_match != 1 else ''}"
        + (f" (a {rate_txt} match rate against the manual estimate)" if rate_txt != '—' else "") + "."
    )
    if n_miss == 0:
        summary_bits.append("Importantly, there were <strong>no genuine misses</strong> — every line that should be derivable from the drawing was produced by the engine.")
    elif n_miss == 1:
        summary_bits.append("There was <strong>one genuine miss</strong> — a drawing-derivable line the engine should have caught but didn't. It's listed below and is the main thing to look at.")
    else:
        summary_bits.append(f"There were <strong>{n_miss} genuine misses</strong> — drawing-derivable lines the engine should have caught but didn't. These are listed first below and are the priority to investigate.")
    if n_scope:
        _ln = "lines are" if n_scope != 1 else "line is"
        summary_bits.append(f"A further <strong>{n_scope}</strong> manual {_ln} logistics or packaging, appearing only on the manual estimate — that's by design, because the engine never invents delivery or packaging from a drawing.")
    if n_extra:
        summary_bits.append(f"The engine also produced <strong>{n_extra}</strong> line{'s' if n_extra != 1 else ''} the estimator didn't have — worth a quick check to confirm whether they belong.")

    verdict_colour = "#2d7a2d" if n_miss == 0 else ("#c88a00" if n_miss <= 2 else "#b33")
    summary_html = (
        f"<div style='background:#fafafa;border-left:4px solid {verdict_colour};border-radius:4px;padding:14px 16px;margin-bottom:18px;line-height:1.6'>"
        + " ".join(summary_bits) +
        "</div>"
    )
    ...
    # ── Count banner ──
    banner = (
        f"<div style='display:flex;gap:14px;flex-wrap:wrap;margin-bottom:18px'>"
        f"<div style='flex:1;min-width:110px;background:#f0f9f0;border:1px solid #cce8cc;border-radius:6px;padding:12px;text-align:center'>"
        f"<div style='font-size:24px;font-weight:700;color:#2d7a2d'>{n_match}</div>"
        f"<div style='font-size:12px;color:#555'>Lines on both</div></div>"
        f"<div style='flex:1;min-width:110px;background:#fdf2f2;border:1px solid #f0cccc;border-radius:6px;padding:12px;text-align:center'>"
        f"<div style='font-size:24px;font-weight:700;color:#b33'>{n_miss}</div>"
        f"<div style='font-size:12px;color:#555'>Genuine misses</div></div>"
        f"<div style='flex:1;min-width:110px;background:#f5f5f5;border:1px solid #ddd;border-radius:6px;padding:12px;text-align:center'>"
        f"<div style='font-size:24px;font-weight:700;color:#888'>{n_scope}</div>"
        f"<div style='font-size:12px;color:#555'>Logistics (by design)</div></div>"
        f"<div style='flex:1;min-width:110px;background:#fff8ec;border:1px solid #f0e0c0;border-radius:6px;padding:12px;text-align:center'>"
        f"<div style='font-size:24px;font-weight:700;color:#c88a00'>{n_extra}</div>"
        f"<div style='font-size:12px;color:#555'>AI only (extra)</div></div>"
        f"<div style='flex:1;min-width:110px;background:#eef4fb;border:1px solid #cdddef;border-radius:6px;padding:12px;text-align:center'>"
        f"<div style='font-size:24px;font-weight:700;color:#2b6cb0'>{rate_txt}</div>"
        f"<div style='font-size:12px;color:#555'>Match rate</div></div>"
        f"</div>"
    )

    out = [summary_html, banner]

    def _narrative_table(rows_data, sentence_fn, cost_key, cost_header):
        body = "".join(
            f"<tr>"
            f"<td style='vertical-align:top;white-space:nowrap'><code>{_esc(r.get('code'))}</code></td>"
            f"<td style='vertical-align:top;line-height:1.55'>{sentence_fn(r)}</td>"
            f"<td style='vertical-align:top;text-align:right;white-space:nowrap'>{_fmt_gbp(r.get(cost_key))}</td>"
            f"</tr>"
            for r in rows_data
        )
        return (f"<div class='table-scroll'><table class='bom-table'><thead><tr>"
                f"<th style='width:120px'>Code</th><th>What this means</th>"
                f"<th style='text-align:right;width:90px'>{cost_header}</th>"
                f"</tr></thead><tbody>{body}</tbody></table></div>")

    # Genuine misses — priority
    if genuine_miss:
        out.append("<h3 style='color:#b33;margin-top:22px'>&#9888; Genuine misses — the engine should have caught these</h3>")
        out.append("<p class='muted'>Each line below was on the manual estimate and is the kind of thing the engine reads from a drawing — a part, fixing, electrical item or length of tube. The engine didn't produce it, so each one is a real gap to close.</p>")
        out.append(_narrative_table(genuine_miss, _miss_sentence, "manual_cost_gbp", "Manual £"))
    else:
        out.append("<h3 style='color:#2d7a2d;margin-top:22px'>&#10003; No genuine misses</h3>")
        out.append("<p class='muted'>Every drawing-derivable line on the manual estimate was also produced by the engine. Nothing to chase here.</p>")

    # Out-of-scope logistics
    if out_of_scope:
        out.append("<h3 style='color:#888;margin-top:22px'>&#8505; Logistics &amp; packaging — added by the estimator, by design</h3>")
        out.append("<p class='muted'>These appear only on the manual estimate, and that is exactly what should happen. They are not engine faults.</p>")
        out.append(_narrative_table(out_of_scope, _scope_sentence, "manual_cost_gbp", "Manual £"))

    # AI-only
    if ai_only:
        out.append("<h3 style='color:#c88a00;margin-top:22px'>On the AI estimate but not the manual estimate</h3>")
        out.append("<p class='muted'>The engine produced these but the estimator didn't have them. A quick look at the drawing tells you whether the engine spotted something genuine or added a line in error.</p>")
        out.append(_narrative_table(ai_only, _ai_only_sentence, "ai_cost_gbp", "AI £"))

    # Matched — value comparison with sentences
    if matched:
        out.append("<h3 style='margin-top:22px'>Lines on both — how the costs compare</h3>")
        out.append("<p class='muted'>These lines appear on both estimates. The sentence explains how closely the two prices agree.</p>")
        body = "".join(
            f"<tr>"
            f"<td style='vertical-align:top;white-space:nowrap'><code>{_esc(r.get('code'))}</code></td>"
            f"<td style='vertical-align:top;line-height:1.55'>{_matched_sentence(r)}</td>"
            f"<td style='vertical-align:top;text-align:right;white-space:nowrap'>{_fmt_gbp(r.get('manual_cost_gbp'))}</td>"
            f"<td style='vertical-align:top;text-align:right;white-space:nowrap'>{_fmt_gbp(r.get('ai_cost_gbp'))}</td>"
            f"</tr>"
            for r in matched
        )
        out.append(
            "<div class='table-scroll'><table class='bom-table'><thead><tr>"
            "<th style='width:120px'>Code</th><th>What this means</th>"
            "<th style='text-align:right;width:90px'>Manual £</th>"
            "<th style='text-align:right;width:90px'>AI £</th>"
            f"</tr></thead><tbody>{body}</tbody></table></div>"
        )
...
def generate_pretty_parity_html(
    *,
    ...
    reconciliation_html = _build_reconciliation_section(bundle.get("bom_set_reconciliation") or {})
...
<div class="page">
  ...
  <!-- SECTION 7: BOM SET RECONCILIATION (three-way) -->
  <div class="section">
    <div class="section-header">
      <h2>7 · Line-by-line reconciliation — what each side has</h2>
      <p>A three-way comparison against the manual estimate (produced by a member of the estimating team). Shows lines on <strong>both</strong>, lines <strong>only on the manual estimate</strong> (the AI missed them — under-scoped), and lines <strong>only on the AI estimate</strong> (extra — verify). This measures completeness, not just whether the totals happen to agree.</p>
    </div>
    <div class="section-body" style="padding:12px 0 0">
      {reconciliation_html}
    </div>
  ...
  <!-- SECTION 8: LABOUR ROUTE -->
  <div class="section">
    <div class="section-header">
      <h2>8 · Labour route — AI hours vs workbook hours per operation</h2>
  ...
  <div class="section">
    <div class="section-header">
      <h2>9 · Risk flags — what needs human attention</h2>
