...
def _call_llm(prompt: str) -> Optional[Any]:
    ...
    The note-scan is a deterministic FINDER, not a reasoner: it passes temperature 0,
    minimal reasoning effort, and a larger completion budget so the reasoning trace can't
    starve the JSON answer (the cause of items intermittently vanishing run-to-run)."""
    try:
        from web_ai_price_lookup import _call_xai_llm
    except Exception:
        return None
    try:
        return _call_xai_llm(
            prompt,
            temperature=0.0,            # pin sampling
            reasoning_effort="low",     # minimise reasoning-trace variation/budget use
            max_completion_tokens=6000, # headroom so the answer is never starved
            seed=7,                     # belt-and-braces determinism if xAI honours it
        )
    except TypeError:
        # Deployed web_ai_price_lookup may predate the override params — fall back safely.
        try:
...
def _price_via_waterfall(code: str, desc: str) -> Optional[Dict[str, Any]]:
   ...
   # Normalise the cache key to SORTED significant tokens of the description so minor LLM
    # wording drift ("50cm lighting loom" vs "lighting loom 50cm") resolves to the SAME
    # cached price — decoupling price determinism from the finder's exact phrasing.
    _norm = " ".join(sorted(_sig_tokens(str(desc) or str(code))))
    _key = _hashlib.md5(("NOTEPRICE|" + _norm).encode("utf-8")).hexdigest()
