...
class PricingService:
    ...
    def _get_db_connection(self):
        ...
        return pyodbc.connect(conn_str, timeout=int(getattr(config, 'SQL_TIMEOUT_DEFAULT_SEC', 30)))
    ...
    def _get_udef_anchor(self, part: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        # Code-prefix bridge: drawing BOM tokens are often a longer word than the UDEF
        # code stem — "ELECTRICS" on the drawing vs "ELEC-50CM LOOM" in UDEF, or a code
        # with spaces ("FIXING 236" vs "FIXING236"). Derive a leading alpha stem (>=4 chars)
        # so a prefix match can bridge the two. Collation is Latin1_General_BIN (case-
        # sensitive), so compare upper-against-upper for the prefix arm.
        import re as _re_udef
        _stem_m = _re_udef.match(r"([A-Za-z]{4,})", part_code)
        code_stem = (_stem_m.group(1) if _stem_m else "")
        code_nospace = part_code.replace(" ", "")
        ...
        row = self._fetch_one_with_retry(
            ...
            WHERE
                ...
                OR UPPER(REPLACE(u.[Part code],' ','')) = UPPER(LTRIM(RTRIM(?)))
                OR (
                    LEN(LTRIM(RTRIM(?))) >= 4
                    AND UPPER(u.[Part code]) LIKE UPPER(LTRIM(RTRIM(?))) + '%'
                )
                OR (
                    LEN(LTRIM(RTRIM(?))) >= 8
            ...
            ORDER BY
                CASE
                    WHEN u.[Part code] = LTRIM(RTRIM(?)) THEN 0
                    WHEN UPPER(REPLACE(u.[Part code],' ','')) = UPPER(LTRIM(RTRIM(?))) THEN 1
                    WHEN UPPER(u.[Part code]) LIKE UPPER(LTRIM(RTRIM(?))) + '%' THEN 2
                    ELSE 3
                END,
                -- Within the prefix/description arms, prefer the shortest (most specific
                -- generic) part code so a bare stem doesn't grab a random expensive variant.
                LEN(u.[Part code]),
                u.[System cost per] DESC
            """,
            [part_code, code_nospace, code_stem, code_stem, desc, desc,
             part_code, code_nospace, code_stem],
        )
        if not row:
            return None
        ...
        base_confidence = float(getattr(config, 'UDEF_CONFIDENCE_EXACT_MATCH', 0.95)) if part_code and part_code == matched_code else float(getattr(config, 'UDEF_CONFIDENCE_DESC_MATCH', 0.82))
    ...
    def _match_quality_label(score: float) -> str:
        if score >= float(getattr(config, 'JACCARD_MIN_SCORE', 0.35)):
            return "strong"
        if score >= 0.15:
            return "moderate"
        return "weak"
    ...
    def _get_pma_purchased(self, part: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        if best_row is None or best_score < float(getattr(config, 'JACCARD_MIN_SCORE', 0.35)):
            return None
        ...
        confidence = min(float(getattr(config, 'PMA_CONFIDENCE_EXACT_MATCH', 0.88)), float(getattr(config, 'PMA_CONFIDENCE_TOKEN_OVERLAP', 0.55)) + best_score * 0.35)
    ...
    def _get_historical_rag(self, part: Dict[str, Any]) -> Dict[str, Any] | None:
        ...
        like_anchor = desc if len(desc) >= len(material) else material
        ...
        if not scored:
            for row in rows:
                if row[1] is not None and float(row[1] or 0) > 0:
    ...
    def _resolve_effective_material_cost(
        ...
        if src == "pma_tbl" and unit_price > 0.0 and confidence >= float(getattr(config, 'PMA_CONFIDENCE_TOKEN_OVERLAP', 0.55)):
        ...
        if (
            ...
            and unit_price > 0.0
            and confidence >= float(getattr(config, 'PMA_CONFIDENCE_TOKEN_OVERLAP', 0.55) + 0.15)
        ):
            system_cost = unit_price * quantity
            return {
                "material_cost_gbp": float(system_cost),
