...
def estimate_material(part: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _is_section_or_wire_candidate(part, material):
        ...
        if is_wire and length_mm:
            ...
            gauge_mm = _safe_float(part.get("normalized_thickness_mm") or _first(part.get("thicknesses_mm", []))) or 3.0
...
def estimate_labour_costs(process: Dict[str, Any], job_quantity: int = 1) -> Dict[str, Any]:
    """
...
def estimate_part(part: Dict[str, Any]) -> Dict[str, Any]:
    ...
    labour = estimate_labour_costs(process, job_quantity=quantity)
    if debug:
