...
def _write_estimate(ws, summary):
    ...
    # Bought-in standard materials — learned from Tim's BOM (job_bought_in_materials.json),
    # not hardcoded: loom, slotted tube, fixings, vinyl, header, packaging. Powder is excluded
    # (the engine computes the powder consumable itself, just above).
    qty = _safe(meta["quantity"])
    _bi_lines, _bi_total = _load_bought_in_for(meta.get("job_number"))
    for _ln in _bi_lines:
        _sc = _ln.get("scrap_pct") or 0
        bom_items.append({
            "desc": _ln.get("description") or _ln.get("part_code") or "",
            "part_code": _ln.get("part_code") or "",
            "supplier": _ln.get("supplier") or "",
            "price": _safe(_ln.get("unit_price_gbp")),
            "qty": _ln.get("qty_per_unit") or 1,
            "scrap": int(round(_sc * 100)) if (_sc and _sc < 1) else int(_sc or 0),
            "total": _safe(_ln.get("total_gbp")),
        })
    ...
    all_mat += _bi_total  # bought-in standard materials (Tim BOM): loom, tube, packaging, fixings...
