...
def _detect_section_stock(text: str) -> Optional[Dict[str, Any]]:
    ...
    (e.g. '30 x 60 x 1.50mm TUBE  1125'), so the estimator's linear-material
    path can fire even when the BOM description is the title-block name
    ('BASE UPRIGHT LEG'). Gated on a hollow-section keyword AND a parseable
    a x b x t profile, so flat-part bend notes (angles, pitches) don't trip it.
    The smallest of the three numbers is taken as the wall thickness."""
    if not text:
        return None
    up = str(text).upper()
    if not any(kw in up for kw in _SECTION_HOLLOW_KW):
        return None
...
def _non_metal_label_from_text(*text_blobs: str) -> str | None:
    ...
    if "200GSM" in joined or ("GSM" in joined and "PAPER" in joined):
        return "200GSM CARD"
    priority = (
        "PETG", "POLYCARBONATE", "PERSPEX", "ACRYLIC", "NYLON", "PVC",
...
_BOM_HEADER_RE = re.compile(
    r"ITEM\s+(?:DWG\s*NO\.?|NO\.?)\s+DESCRIPTION\s+QTY",
    re.IGNORECASE,
)
