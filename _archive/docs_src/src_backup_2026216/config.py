...
HOURLY_RATES_GBP = {
    "laser_cutting": 68.19,
    "laser_cutting_acrylic": 41.21,
    "hole_machining": 43.77,
    "folding": 40.47,
    "powder_coating": 355.43,
    "handling": 31.18,
    "assembly": 28.56,
    "assembly_acrylic": 25.43,
    "welding": 41.77,
    "wire_forming": 39.84,
    "spot_welding": 32.90,
    "resistance_welding": 32.90,
    "deburring": 31.18,
    "guillotine": 31.79,
    "diamond_polish": 31.60,
    "dress_welds": 28.68,
    "glue": 25.43,
    "punch": 41.77,
    "roll": 39.84,
    "saw": 31.80,
    "linisher": 25.43,
    "manual_labour_metal": 31.18,
    "manual_labour_acrylic": 25.43,
    "cnc": 43.39,
    "cnc_routing": 43.39,
    "wet_spray": 33.54,
    "bench_work": 31.18,
...
SELECT TOP (1) * FROM (
    ...
    WHERE
        ...
        OR u.[Description] LIKE '%' + LTRIM(RTRIM(?)) + '%'
    UNION ALL
    SELECT
    ...
    WHERE b.is_active = 1
      AND (
          b.part_code = LTRIM(RTRIM(?))
          OR b.description LIKE '%' + LTRIM(RTRIM(?)) + '%'
      )
) combined
ORDER BY
    CASE WHEN combined.part_code = LTRIM(RTRIM(?)) THEN 0 ELSE 1 END,
    source_rank,
    combined.price DESC
""",
    },
...
WORKBOOK_INPUT_DEFAULTS = {
    ...
    "wire_cost_per_tonne_gbp": float(os.getenv("WORKBOOK_WIRE_COST_PER_TONNE_GBP", "1600.0")),
    # L5 in workbook: Sheet Steel Cost Per Tonne. Blank sheet = £900. Override via env or workbook scan.
    "sheet_steel_cost_per_tonne_gbp": float(os.getenv("WORKBOOK_SHEET_STEEL_COST_PER_TONNE_GBP", "900.0")),
...
POWDER_COSTING_POLICY = {
    ...
    "powder_material_gbp_per_kg": float(os.getenv("POWDER_MATERIAL_GBP_PER_KG", "10.0")),
    ...
    "powder_material_gbp_per_kg_special": float(os.getenv("POWDER_MATERIAL_SPECIAL_GBP_PER_KG", "16.0")),
