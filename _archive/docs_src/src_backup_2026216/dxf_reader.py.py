...
CUT_LAYERS  = {"SLD-0", "0"}                          # outline / laser cut profile
BEND_LAYERS = {"BENDLINES", "BEND", "BEND_LINES"}     # press brake fold axes
ETCH_LAYERS = {"ETCHING", "ETCH", "SCRIBE"}           # etching / scribe marks
SKIP_LAYERS = {                                        # decorative / non-geometry
...
def _exact_perimeter_and_area(
    ...
    cut-outline layer.  Lines are ordered for the shoelace; arcs add their
...
def extract_flat_pattern_data(dxf_path: Path) -> Dict[str, Any]:
    ...
    outline  = _exact_perimeter_and_area(cut_lines, cut_arcs, scale)
