...
DRAWING_JOB_DISCOVERY = {
    ...
    # Treat every PDF in the same folder as one job: pool BOM, match all folder DXFs once,
    # emit one combined bay estimate (see file_scan.scan_folder_job).
    "folder_as_job": os.getenv("FOLDER_AS_JOB", "1").lower() in {"1", "true", "yes"},
}
...
WORKBOOK_INPUT_DEFAULTS = {
    ...
    "sheet_steel_cost_per_tonne_gbp": float(os.getenv("WORKBOOK_SHEET_STEEL_COST_PER_TONNE_GBP", "800.0")),  # SDI rate £800/tonne
