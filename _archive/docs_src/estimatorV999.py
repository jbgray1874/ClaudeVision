...
def estimate_material(part: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # A stated/DXF weight must NOT override the sheet-steel geometry formula.
    # This path is for timber/wood (where extracted thickness is a tolerance
    # artefact) and for parts with no usable blank geometry. For sheet metal with
    # a reliable flat blank, geometry is the trusted path — and DXF/title-block
    # weights are frequently junk (e.g. a 0.30 m^2 x 1mm steel panel reported as
    # 0.71 g), which silently zeroes the material cost. So: skip the stated weight
    # for sheet metal that has geometry, and otherwise sanity-check it against the
    # geometric mass before trusting it.
    _use_stated_weight = stated_weight_kg is not None and stated_weight_kg > 0
    if _use_stated_weight:
        _mat_u = (material or "").upper()
        _sheet_metals = {
            ...
            "ALUMINIUM", "ALUMINUM", "BRIGHT_DRAWN",
        }
        _have_geom = bool(blank_length and blank_width and thickness)
        _geom_kg = None
        if _have_geom:
            _dens = (MATERIAL_DENSITY_KG_PER_M3.get(material)
                     or MATERIAL_DENSITY_KG_PER_M3.get(_mat_u))
            if _dens:
                _geom_kg = (blank_length * blank_width / 1_000_000.0) * (thickness / 1000.0) * _dens
        if _mat_u in _sheet_metals and _have_geom:
            # geometry is the trusted path for sheet metal — ignore stated weight
            _use_stated_weight = False
            part.setdefault("review_flags", []).append({
                "severity": "info",
                "flag": "stated_weight_ignored_sheet_metal",
                "detail": f"stated/DXF weight {round(stated_weight_kg, 4)}kg ignored for sheet metal; using sheet-steel geometry formula",
            })
        elif (
            (part.get("dxf_augmented") or part.get("flat_pattern_detected"))
            and _geom_kg and _geom_kg > 0
            and (stated_weight_kg < 0.3 * _geom_kg or stated_weight_kg > 3.0 * _geom_kg)
        ):
            # implausible vs geometry (e.g. 0.71 g for a part that should weigh kg)
            _use_stated_weight = False
            part.setdefault("review_flags", []).append({
                "severity": "warning",
                "flag": "stated_weight_implausible",
                "detail": f"stated weight {round(stated_weight_kg, 4)}kg vs geometric ~{round(_geom_kg, 3)}kg — ignored, using geometry",
            })
    if _use_stated_weight:
        applied_price_per_kg = external_price.get("applied_price_per_kg")
        fallback_price_per_kg = MATERIAL_PRICE_GBP_PER_KG.get(material or "")
