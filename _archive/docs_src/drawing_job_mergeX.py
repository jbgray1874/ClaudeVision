...
def apply_dxf_geometry_to_part(part: Dict[str, Any], dxf_path: Path) -> Dict[str, Any]:
    ...
    # its flat DXF) arrive here with geometry but no operations, so they cost \u00a30. Stamp
    # baseline fab ops inferred from the flat geometry \u2014 cut length -> laser/profile cut,
    # bend lines -> folding \u2014 so they are costed like any other flat. Parts that already
    # carry operations from a detail page are left untouched.
    if not (part.get("operations") or []):
        try:
        ...
        if cut_len > 0:
            try:
                ...
                if inferred:
                    part["operations"] = sorted(set(inferred))
                    part["textual_operations"] = list(part["operations"])
