...
def codes_shadowed_by_parent_bom(bom_rows: List[Dict[str, Any]], est_codes: Iterable[str]) -> set:
    ...
    for row in bom_rows:
        ...
        try:
            ...
            pass
        if resolved:
            resolved_norm = _norm_code(resolved)
            # Only shadow when the row resolves to a DIFFERENT code (a real parent->child
            # link, e.g. 1450-GA -> 1450-01C). A row that resolves to its own estimate is
            # the line itself, not a shadowed child; self-shadowing was dropping legitimate
            # top-level synthesized lines (1449-01C, 2621-01C, header) in folder-as-job mode.
            if resolved_norm and resolved_norm != _norm_code(code):
                shadowed.add(resolved_norm)
                continue
        ...
        if kind == "catalogue":
            ...
            if code in est_by_code:
                ...
                def _is_child(c: str) -> bool:
                    if c in bound_detail_codes:
                        return False
                    # Normally a child with its own exact BOM row is excluded to avoid
                    # double-count. But if that row was shadowed (skipped) by THIS parent,
                    # it must be rolled up here or the part vanishes from the bay entirely.
                    if c in claimed_exact and c not in shadowed_details:
                        return False
                    if c == code:
                        return False
