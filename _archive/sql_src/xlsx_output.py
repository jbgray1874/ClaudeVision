...
def _write_estimate(ws, summary):
    ...
    # ── Basis & Provisos: surface the stamped AI working so the team can challenge it ──
    ROW += 2
    _section_hdr(ws, ROW, 1, "Basis & Provisos  (AI working — verify before quoting)", merge_to=13)
    ROW += 1
    _wrote_basis = False
    for pe in pes:
        notes = []
        pc = pe.get("punch_calibration") or {}
        if pc.get("basis"):
            notes.append(str(pc["basis"]))
        sca = pe.get("section_costing_adjustment") or {}
        if sca.get("note"):
            notes.append(str(sca["note"]))
        for rf in (pe.get("review_flags") or []):
            if isinstance(rf, dict) and rf.get("detail"):
                notes.append(f"{rf.get('flag','flag')}: {rf['detail']}")
        if not notes:
            continue
        _wrote_basis = True
        _set(ws, ROW, 1,
             f"{pe.get('part_number','')}  {pe.get('description') or ''}".strip(),
             bold=True, size=9, fill=_BLUE_LIGHT)
        ws.merge_cells(f"A{ROW}:C{ROW}")
        _set(ws, ROW, 4, "   •   ".join(notes), size=9, italic=True, colour="595959")
        ws.merge_cells(f"D{ROW}:M{ROW}")
        ROW += 1
    if not _wrote_basis:
        _set(ws, ROW, 1, "No inferred or provisional lines — all costs derive from direct data.",
             size=9, italic=True, colour="595959")
