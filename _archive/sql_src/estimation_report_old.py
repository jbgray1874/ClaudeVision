"""
SDIAIVision — Estimation Provenance Report
===========================================
...
def build_provenance(summary: Dict[str, Any]) -> List[Dict]:
    ...
    for part in parts:
        ...
        mat  = part.get("normalized_material") or "Unknown"
        qty  = part.get("quantity", 1)
        unit = float(part.get("unit_estimate") or 0)
        ext  = float(part.get("extended_estimate") or 0)
        geo  = str(part.get("geometry_source") or "pdf")
        ops  = part.get("operations") or []
        ...
        thk_clean    = [t for t in thicknesses
                        if t and round(float(t), 1) not in tol_set] if thicknesses else []
        thk_val      = thk_clean[0] if thk_clean else None
        thk_source   = "DXF filename" if part.get("dxf_source_file") and thk_val else \
                       "DXF geometry" if "dxf" in geo and thk_val else \
                       "PDF text" if thk_val else \
                       "Not extracted (tolerance table only)"
        thk_conf     = 0.95 if "dxf" in thk_source.lower() else \
                       0.7  if thk_val else 0.2

        # ── Geometry provenance ────────────────────────────────────────────────
        geo_data     = part.get("geometry") or {}
...
def add_provenance_sheet(wb, summary: Dict[str, Any],
    ...
    cell(1, 1, "SDIAIVision — Estimate Provenance Report",
    ...
    cell(row, 1,
         ...
         f"Generated by SDIAIVision  |  {datetime.now().strftime('%d/%m/%Y %H:%M')}",
...
def generate_standalone_report(summary: Dict[str, Any],
    ...
    # Simple cover page
    ws["A1"] = "SDIAIVision Estimation Provenance Report"
...
if __name__ == "__main__":
    print("SDIAIVision Provenance Report module ready.")
    print()
