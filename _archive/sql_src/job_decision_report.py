...
def _ops_explanation(part: Dict) -> str:
...
def add_decision_report_sheet(wb, summary: Dict[str, Any],
    ...
    for i, part in enumerate(parts):
        ...
        thks     = part.get("thicknesses_mm") or []
        tol      = {0.5, 1.0, 1.5, 2.0, 3.0}
        if _m_t and 0.3 <= float(_m_t.group(1)) <= 25.0:
        ...
        if conf < 0.5 or unit == 0 and mat not in ("BOUGHT_IN",):
            review_parts.append((pn, desc, conf_expl))

    # ── Total row ──────────────────────────────────────────────────────────────
    row += 1
    ws.merge_cells(f"A{row}:H{row}")
    _c(ws, row, 1, "TOTAL ESTIMATE", bold=True, bg=C_NAVY, fg=C_WHITE,
       align="right", size=12)
