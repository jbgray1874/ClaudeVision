...
def _build_gaps_explanation(
    ...
    # Missing prices
    missing = [p for p in parts if not (p.get("material_estimate") or {}).get("applied_price_per_kg_gbp") and not (p.get("material_estimate") or {}).get("material_price_per_kg_gbp")]
    if missing:
        pn_list = ", ".join(_esc(str(p.get("part_number") or "?")) for p in missing[:6])
        items.append(
            f"<li class='gap-item gap-high'>❌ <strong>{len(missing)} part(s) have no material price at all: {pn_list}.</strong> "
            "These parts are uncosted — their contribution to the total is zero. "
            "Add the material to MATERIAL_PRICE_GBP_PER_KG in config.py or to the SQL material_prices table.</li>"
        )
...
<div class='scorecard {status_cls}'>
  ...
  <p class='score-note'>Money cells matched: {pct}% · Cells checked: {exec_s.get('money_total',0)}</p>
</div>"""
