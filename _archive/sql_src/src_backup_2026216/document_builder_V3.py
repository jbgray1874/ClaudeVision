...
def _build_process_routing(part: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if "laser_cutting" in operations:
        routing.append({"operation": "laser_cutting", "phase": "profile", "driver": "cut_length_and_pierces"})
    if "hole_machining" in operations:
        routing.append({"operation": "hole_machining", "phase": "secondary", "driver": "hole_count"})
    if "tapping" in operations:
        routing.append({"operation": "tapping", "phase": "secondary", "driver": "thread_features"})
    if "countersinking" in operations:
        routing.append({"operation": "countersinking", "phase": "secondary", "driver": "csk_features"})
    if "folding" in operations:
        routing.append({"operation": "folding", "phase": "forming", "driver": "bend_count_and_length"})
    if "welding" in operations:
        routing.append({"operation": "welding", "phase": "assembly", "driver": "weld_notes"})
    if "powder_coating" in operations:
        routing.append({"operation": "powder_coating", "phase": "finish", "driver": "finish_requirement"})
    routing.append({"operation": "handling", "phase": "logistics", "driver": "part_count"})
    return routing
...
def _interpret_part(part: Dict[str, Any]) -> Dict[str, Any]:
    feature_rollup = {
        ...
        "bend_feature_count": max(len(part.get("angles_deg", [])), len(part.get("fold_values_mm", [])), part["geometry_rollup"].get("estimated_bend_line_count", 0)),
        "radius_feature_count": len(part.get("radii_mm", [])),
        "tap_feature_count": 1 if "tapping" in part.get("textual_operations", []) else 0,
        "weld_feature_count": 1 if "welding" in part.get("textual_operations", []) else 0,
    }
    ...
    part["manufacturing_interpretation"] = {
        ...
        "process_family": "fabrication" if part.get("textual_operations") else "review_required",
        "setup_driven_operations": [step["operation"] for step in _build_process_routing(part) if step["operation"] in {"laser_cutting", "hole_machining", "folding"}],
        "run_driven_operations": [step["operation"] for step in _build_process_routing(part) if step["operation"] in {"laser_cutting", "hole_machining", "folding", "powder_coating", "handling"}],
    }
    return part
