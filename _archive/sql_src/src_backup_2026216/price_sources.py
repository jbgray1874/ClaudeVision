...
def _fetch_candidates_from_connector(connector: Any, request: PriceRequest) -> List[PriceCandidate]:
    ...
    if request.kind == "material_price" and hasattr(connector, "get_material_price"):
        ...
        try:
            rows = gm(request.material or "", request.thickness_mm, request.quantity, request.description or "")
        except TypeError:
            rows = gm(request.material or "", request.thickness_mm, request.quantity)
