...
def main() -> None:
    ...
    if args.merge_dxf_into:
        ...
        dxf_paths: list[Path] = [Path(p) for p in (args.attach_dxf or [])]
        if args.dxf_glob:
    ...
    if not files:
        print(f"No drawing files found in {args.search_root if not drawing_arg else drawing_arg}")
        return
    ...
    attach_dxf = [Path(p) for p in (args.attach_dxf or [])]
    ...
    for pdf_path in files:
        ...
        summary, output_paths = scan_file(
            pdf_path,
            attach_dxf_paths=attach_dxf or None,
