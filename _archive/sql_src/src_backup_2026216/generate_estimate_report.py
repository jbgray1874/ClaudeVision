...
.badge-config{ background: #F3F4F6; color: #6B7280; }
    .badge-manual { background: #FEE2E2; color: #991B1B; font-weight:700; }
    .needs-manual > td { background: #FFF5F5 !important; }
    .gap-banner { background:#FEF2F2; border:2px solid #FCA5A5;
                  border-radius:8px; padding:16px 20px; margin-bottom:20px; }
    .gap-banner h3 { color:#991B1B; margin-bottom:8px; font-size:14px; }
    .gap-table { width:100%; border-collapse:collapse; font-size:12px; margin-top:8px; }
    .gap-table th { background:#FECACA; color:#7F1D1D; padding:6px 10px; text-align:left; }
    .gap-table td { padding:6px 10px; border-bottom:1px solid #FEE2E2; }
...
_GAP_REASONS: Dict[str, str] = {
    "no_blank_dims":   "No blank dimensions — material cost = £0 (DXF needed or manual measure)",
    "zero_bought_in":  "Bought-in part not in UDEF/catalogue — manual price required",
    "zero_total":      "No cost calculated — manual pricing required",
    "print_graphics":  "Print / graphics — always manual (supplier quote needed)",
    "powder_coating":  "Powder coating — process cost; check labour route or add to catalogue",
    "timber_laminate": "Timber / laminate — no unit price seeded (add to supplier catalogue)",
}

_GAP_BADGE_LABELS: Dict[str, str] = {
    "no_blank_dims": "⚠ No blank dims",
    "zero_bought_in": "⚠ Not in UDEF",
    "zero_total": "⚠ No cost",
    "print_graphics": "⚠ Print/graphics",
    "powder_coating": "⚠ Powder coat",
    "timber_laminate": "⚠ Timber/laminate",
}


def _mat_candidates(part: Dict, d: Dict) -> str:
    """Uppercased material blob: mfg writeup, material_estimate, normalized_material, description."""
    mpart = _get_mfg_part(d, str(part.get("part_number") or ""))
    bits: List[str] = []
    mat = mpart.get("materials")
    if isinstance(mat, list):
        bits.extend(str(x) for x in mat if x)
    elif isinstance(mat, str) and mat:
        bits.append(mat)
    me = part.get("material_estimate") or {}
    if me.get("material"):
        bits.append(str(me["material"]))
    if part.get("normalized_material"):
        bits.append(str(part["normalized_material"]))
    if part.get("description"):
        bits.append(str(part["description"]))
    return " ".join(bits).upper()


def _gap_badge_label(reason_key: str) -> str:
    return _GAP_BADGE_LABELS.get(reason_key, "⚠ Manual price needed")


def _gap_stat_tile(count: int) -> str:
    if count <= 0:
        return ""
    return f"""  <div class="stat" style="border-color:#FCA5A5;background:#FEF2F2">
    <div class="s-label" style="color:#991B1B">⚠ Need manual price</div>
    <div class="s-value" style="color:#991B1B">{count}</div>
  </div>
"""


def _resolve_project_root(script_path: Path) -> Path:
    """Repo root when script lives under src/; else CWD if it looks like the project."""
    here = script_path.resolve().parent
    if here.name == "src" and (here.parent / "src").is_dir():
        return here.parent
    cwd = Path.cwd()
    if (cwd / "src").is_dir():
        return cwd
    return here.parent if here.name == "src" else cwd


def _needs_manual_price(part: Dict, d: Dict) -> Tuple[bool, str]:
    """Return (needs_manual, reason_key). Assembly-only placeholders are excluded."""
    if _is_assembly_only_placeholder(part, d):
        return False, ""

    pn = str(part.get("part_number") or "").upper()
    cost = float(part.get("unit_total_cost_gbp") or 0)
    cb = part.get("cost_breakdown") or {}
    me = part.get("material_estimate") or {}
    sc = cb.get("system_cost") or {}
    mat = _mat_candidates(part, d)

    # Print / graphics
    if any(x in pn for x in ("PRINT", "GRAPHIC", "VINYL", "SIGN", "GFX")):
        return True, "print_graphics"

    # Powder coating
    if "POWDER" in mat or "P/C" in pn:
        return True, "powder_coating"

    # Timber / laminates
    if any(x in mat for x in ("TIMBER", "LAMINATE", "VENEER", "BOARD")):
        return True, "timber_laminate"

    # Bought-in with no priced source
    if _is_bought_in(part, d):
        if not sc.get("applied_to_total") and cost < 0.01:
            return True, "zero_bought_in"

    # Fabricated steel/acrylic with zero material cost
    mat_cost = float(
        me.get("unit_material_cost_gbp")
        or (cb.get("material") or {}).get("unit_material_cost_gbp")
        or 0
    )
    if mat_cost == 0 and not _is_bought_in(part, d):
        if any(x in mat for x in ("STEEL", "ACRYLIC", "PETG", "POLYCARB", "ALUMIN")):
            return True, "no_blank_dims"

    # Fallback: zero total for non-assembly
    if cost == 0 and not _is_assembly_only_placeholder(part, d):
        return True, "zero_total"

    return False, ""


def _gap_banner(gaps: List[Tuple[str, str, str]]) -> str:
    """Red banner listing all parts needing manual pricing."""
    if not gaps:
        return ""
    rows = "".join(
        f"<tr><td><strong>{pn}</strong></td><td>{desc}</td>"
        f"<td style='color:#991B1B'>{_GAP_REASONS.get(rk, 'Manual price required')}</td></tr>"
        for pn, desc, rk in gaps
    )
    return f"""
<div class="gap-banner">
  <h3>⚠ {len(gaps)} Part{"s" if len(gaps) != 1 else ""} Require Manual Pricing Before Quoting</h3>
  <p style="font-size:12px;color:#7F1D1D;margin-bottom:10px;">
    These parts have no automated price source. The estimate total EXCLUDES their cost.
    Add prices to UDEF, supplier catalogue, or enter manually before issuing a quote.
  </p>
  <table class="gap-table">
    <thead><tr>
      <th>Part number</th><th>Description</th><th>Action required</th>
    </tr></thead>
    <tbody>{rows}</tbody>
  </table>
</div>
...
def build_html(d: Dict, source_file: str) -> str:
    ...
    _all_gaps: List[Tuple[str, str, str]] = []
    for _gp in parts:
        _gn, _gr = _needs_manual_price(_gp, d)
        if _gn:
            _all_gaps.append(
                (str(_gp.get("part_number") or ""), str(_gp.get("description") or ""), _gr)
            )
...
{_gap_stat_tile(len(_all_gaps))}</div>
""")

    h.append(_gap_banner(_all_gaps))
...
""")
    ...
    for p in parts:
        ...
        _gap_flag, _gap_reason = _needs_manual_price(p, d)
        if _is_bought_in(p, d):
            row_class = "bought-in needs-manual" if _gap_flag else "bought-in"
            ...
            row_class = "needs-manual" if _gap_flag else ""
            role_tag = '<span class="role role-detail">Fabricated</span>'
        if _gap_flag:
            badge_label = _gap_badge_label(_gap_reason)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    repo_root = _resolve_project_root(Path(__file__))
