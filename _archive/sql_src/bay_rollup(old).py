...
def synthesize_folder_job_bom_rows(
    ...
    return dedupe_weldment_parent_rows(rows)
...
def build_bay_estimate(
    ...
    *,
    ...
    for row in bom_rows:
        ...
        if kind == "catalogue":
            priced = catalogue_pricer(code, desc) if catalogue_pricer else None
...
def make_system_cost_pricer(resolve_part_system_cost, *, reject_above_gbp: float = 750.0):
    ...
    def pricer(code: str, desc: str):
        part = {"part_number": code, "description": desc}
        try:
        ...
        try:
            ...
            return {"unit_cost_gbp": None, "source": "system_cost_no_match", "reason": "non-numeric price"}
        if uc <= 0 or uc > reject_above_gbp:
            return {
                "unit_cost_gbp": None,
                "source": "system_cost_rejected",
                "reason": f"implausible match £{uc:.0f} (>£{reject_above_gbp:.0f}) — rejected",
