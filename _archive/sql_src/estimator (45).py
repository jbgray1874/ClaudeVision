...
def estimate_material(part: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Sanity gate: for sheet stock with a credible flat blank, a stated weight wildly
    # inconsistent with area x thickness x density is title-block OCR garbage (e.g. a
    # concatenated dimension string read as "1151083g"). Reject it and fall through to the
    # sheet-nesting formula. Stated weight is still trusted for non-sheet stock (timber/
    # wood/MDF) where extracted thickness is unreliable, and when no blank is known.
    if (
        stated_weight_kg is not None
        and stated_weight_kg > 0
        and (material or "").upper() not in _NON_SHEET_MATERIALS
        and blank_length and blank_width and thickness
    ):
        _density = (
            MATERIAL_DENSITY_KG_PER_M3.get(material)
            or MATERIAL_DENSITY_KG_PER_M3.get((material or "").upper(), 7850.0)
        )
        _expected_kg = (blank_length * blank_width) / 1_000_000.0 * (thickness / 1000.0) * _density
        _max_ratio = float(
            (getattr(config, "MATERIAL_PRICE_POLICY", {}) or {}).get("max_stated_weight_blank_ratio", 3.0)
        )
        if _expected_kg > 0 and stated_weight_kg > _max_ratio * _expected_kg:
            stated_weight_kg = None  # implausible weight — use the sheet formula instead
