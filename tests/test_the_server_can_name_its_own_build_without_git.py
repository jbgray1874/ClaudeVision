...
`.sdi-commit` beside the code it copied, and the start scripts read it when git is
unavailable. The answer travels with the deploy instead of being recomputed where it cannot
be. app.py already prefers `SDI_COMMIT` over its own git lookup, so nothing there changed.
